# 등장인물 색인

이 문서는 소설의 주요 의인화 인물을 한눈에 보기 위한 색인이다.

## 핵심 주역

| 인물 | 역할 | 성격 한 줄 | 관련 파일 |
|---|---|---|---|
| Bitcoin | 규칙 기반 화폐의 원형 | 완고하고 드물게 움직이지만, 한 번 정한 규칙은 쉽게 꺾지 않는다 | [Bitcoin](./Bitcoin.md) |
| Ethereum | 프로그래머블 시티의 설계자 | 가능성을 먼저 보는 발명가에서 복잡한 도시의 책임자로 성장한다 | [Ethereum](./Ethereum.md) |
| Stablecoins | 가치 고정과 유동성의 관리자 | 요란하지 않지만 늘 시스템 한가운데에 있는 회계 담당자 | [Stablecoins](./Stablecoins.md) |
| 신용 | 미래 약속과 레버리지 | 사라지지 않고 새로운 기술 안에서 계속 다른 옷을 입는 오래된 상대역 | [Credit](./Credit.md) |
| 가격 | 군중의 기대를 숫자로 압축 | 진실을 말하지 않고 사람들이 지금 무엇에 얼마를 거는지만 반영한다 | [Price](./Price.md) |
| 국가 | 법·강제력·권리구제 | 느리고 내부가 모순적이지만 코드 밖 현실의 책임을 묻는다 | [State](./State.md) |
| Institutions | 제도권 자본과 위임책임 | 철학보다 관리 가능한 위험과 고객 수요를 계산한다 | [Institutions](./Institutions.md) |
| Exchanges | 접근성과 유동성의 관문 | 편리함을 파는 만큼 열쇠와 권력이 모이는 장사꾼 | [Exchanges](./Exchanges.md) |
| XRP | 속도와 연결의 외교관 | 철학보다 결제와 이동의 효율을 중시하는 실무가 | [XRP](./XRP.md) |
| Solana | 고속 신도시의 건축가 | 빠르고 야심차며 규모에 집착하는 젊은 도전자 | [Solana](./Solana.md) |
| Chainlink | 외부 세계를 전하는 오라클 | 말수는 적지만 모든 계약이 필요로 하는 정보 중개자 | [Chainlink](./Chainlink.md) |

## 실제 인간 핵심 카드

실존 인물은 의인화 프로토콜과 분리한다. 공개 기록으로 확인되지 않은 외모·심리·사적 대사는 정본 사실로 만들지 않는다.

| 인물 | 역사적 기능 | 관련 파일 |
|---|---|---|
| Satoshi Nakamoto | Bitcoin 백서·초기 소프트웨어 공개 | [Satoshi Nakamoto](./Satoshi_Nakamoto.md) |
| Hal Finney | 초기 Bitcoin 실행·시험 전송의 공개 기록 | [Hal Finney](./Hal_Finney.md) |
| Vitalik Buterin | Ethereum 초기 백서·프로젝트 공개 | [Vitalik Buterin](./Vitalik_Buterin.md) |

## 초상화 갤러리

> 시각 정본은 [`CHARACTER_ART_BIBLE`](./CHARACTER_ART_BIBLE.md)을 따른다. 아래 6개 SVG는 같은 4:5 벡터 캐릭터 체계로 제작된 정본 일러스트다.

### Bitcoin
![Bitcoin Character](./portraits/bitcoin.svg)

### Ethereum
![Ethereum Character](./portraits/ethereum.svg)

### XRP
![XRP Character](./portraits/xrp.svg)

### Stablecoins
![Stablecoins Character](./portraits/stablecoins.svg)

### Solana
![Solana Character](./portraits/solana.svg)

### Chainlink
![Chainlink Character](./portraits/chainlink.svg)

### 초상화 범위 규칙
- 정본 방향은 실사 초상화가 아니라 **그래픽노블/편집 일러스트 계열의 캐릭터 디자인**이다.
- 캐릭터는 실제 창립자의 외모가 아니라 프로토콜·자산의 성격, 역할, 반복 모티프를 시각화한다.
- 이름이나 로고 없이도 실루엣·복장·소품·공간 언어로 서로 구별되어야 한다.
- Satoshi Nakamoto는 신원·외모가 확인되지 않았으므로 정본 초상화를 만들지 않는다.
- Hal Finney, Vitalik Buterin 등 실존 인물은 의인화 프로토콜 캐릭터 세트와 분리한다.
- `신용`, `가격`, `국가`, `Institutions`, `Exchanges`는 특정 실존 인물의 얼굴로 고정하지 않는다. 필요하면 별도의 개념 캐릭터로 디자인한다.

## 복합 등장인물 사용 규칙
- **Exchanges**: 여러 중앙화 거래소의 반복되는 기능을 의인화한다. 사건 장면에서는 Mt. Gox, Binance, FTX 등 실제 조직의 사실관계를 분리한다.
- **Institutions**: 은행·자산운용사·수탁사·브로커 등 위임된 자본의 군중이다. 정부·법원·규제기관의 강제력은 `국가`와 분리한다.
- **국가**: 단일 정부나 정당이 아니다. 기관·법원·의회·행정부가 서로 충돌할 수 있는 복합 인물이다.
- **Stablecoins**: 한 설계가 아니라 가족이다. USDT, USDC, DAI, UST를 같은 구조로 취급하지 않는다.

## 집필용 기준표
캐릭터별 대사 충돌과 반복 모티프의 담당은 [`PASS2_CHARACTER_MATRIX`](../10_REVISION/PASS2_CHARACTER_MATRIX.md)를 정본으로 사용한다.

## 추가 세부 카드 후보
- Mt. Gox, The DAO, Tether, Circle, Ripple, Binance, FTX, Terra/Luna 등 조직·프로토콜 카드.
- Bitcoin Cash, Ethereum Classic, USDT, USDC, TerraUSD 등 분기·파생 인물 카드.

이 후보들은 장별 사실·연속성 검수에서 독립 dossier가 필요하다고 판정될 때 추가한다. 현재의 publication-ready 완료 조건에서 자동 필수 항목으로 간주하지 않는다.

> 초상화는 실제 창립자의 외모를 재현하는 이미지가 아니라, 각 프로토콜/자산의 성격을 읽히게 만드는 소설용 캐릭터 일러스트다.
